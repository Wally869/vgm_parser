...
use crate::{HeaderData, VgmMetadata, Commands};
...
impl OffsetValidator {
    ...
    pub fn validate_offset(offset: u32, file_size: usize, field_name: &str) -> VgmResult<()> {
        let offset_usize = usize::try_from(offset)
            .map_err(|_| VgmError::InvalidOffset {
    ...
    pub fn validate_range(offset: u32, size: u32, file_size: usize, field_name: &str) -> VgmResult<()> {
...
impl ChipValidator {
    ...
    fn validate_clock_range(clock: u32, chip_name: &str, min_hz: u32, max_hz: u32) -> VgmResult<()> {
        if clock > 0 && (clock < min_hz || clock > max_hz) {
            return Err(VgmError::ValidationFailed {
                field: format!("{} clock", chip_name),
                reason: format!("Clock {} Hz outside valid range {}-{} Hz", clock, min_hz, max_hz),
            });
        }
        Ok(())
    }
    ...
    pub fn validate_chip_volumes(header: &HeaderData) -> VgmResult<()> {
        ...
        if header.volume_modifier > 64 {
            return Err(VgmError::ValidationFailed {
                ...
                reason: format!("Volume modifier {} exceeds maximum 64", header.volume_modifier),
            });
        }
...
impl ConsistencyValidator {
    ...
    pub fn validate_header_consistency(header: &HeaderData, file_size: usize) -> VgmResult<()> {
        ...
        if header.vgm_data_offset > 0 {
            OffsetValidator::validate_offset(header.vgm_data_offset + 0x34, file_size, "vgm_data_offset")?;
        }
        ...
        if header.extra_header_offset > 0 {
            OffsetValidator::validate_offset(header.extra_header_offset + 0xBC, file_size, "extra_header_offset")?;
        }
    ...
    pub fn validate_commands_consistency(header: &HeaderData, commands: &[Commands]) -> VgmResult<()> {
...
impl ChipUsageTracker {
    ...
    fn track_command(&mut self, command: &Commands) {
        match command {
            ...
            Commands::YM2612Port0Write { .. } | Commands::YM2612Port1Write { .. } => self.ym2612_used = true,
            ...
            Commands::YM2608Port0Write { .. } | Commands::YM2608Port1Write { .. } => self.ym2608_used = true,
            Commands::YM2610Port0Write { .. } | Commands::YM2610Port1Write { .. } => self.ym2610_used = true,
