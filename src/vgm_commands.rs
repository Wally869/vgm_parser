...
impl Commands {
    pub fn to_bytes(self) -> Vec<u8> {
        match self {
            ...
            Commands::WaitNSamplesPlus1 { n } => return vec![0x70 + n],
            ...
            Commands::YMF278BWrite {
                ...
                return vec![0xD0, port, register, value];
            }
            ...
            Commands::YMF271Write {
                ...
                return vec![0xD1, port, register, value];
            }
            Commands::SCC1Write {
                port,
                register,
                ...
                return vec![0xD2, port, register, value];
            }
            Commands::K054539Write { register, value } => {
                let temp = register.to_le_bytes();
                return vec![0xD3, temp[0], temp[1], value];
            }
            Commands::C140Write { register, value } => {
                let temp = register.to_le_bytes();
                return vec![0xD4, temp[0], temp[1], value];
            }

            Commands::ES5503Write { register, value } => {
                let temp = register.to_le_bytes();
                return vec![0xD5, temp[0], temp[1], value];
            }
            Commands::ES5506Write16 { register, value } => {
                let temp = value.to_le_bytes();
                return vec![0xD6, register, temp[0], temp[1]];
            }
