...
mod integration_tests {
    ...
    use crate::{VgmFile, HeaderData, VgmMetadata, Gd3LocaleData, Commands};
    use bytes::Bytes;
    ...
    fn test_validation_framework_integration() {
        ...
        assert!(ConsistencyValidator::validate_commands_consistency(&inconsistent_header, &commands_with_ym2612).is_err());
        
        // Test 6: Valid consistency should pass
        inconsistent_header.ym2612_clock = 7670453; // Add clock configuration
        assert!(ConsistencyValidator::validate_commands_consistency(&inconsistent_header, &commands_with_ym2612).is_ok());
    }
    ...
    fn test_validation_config_limits() {
        ...
        assert!(VersionValidator::validate_version(0x170, &strict_config).is_err()); // Too new
    }
    ...
    fn test_comprehensive_vgm_validation() {
        ...
        assert!(strict_validator.validate_vgm_file(&header, &commands, &metadata, file_size).is_ok());
    }
}