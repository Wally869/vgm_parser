...
pub enum VgmError {
    ...
    InvalidUtf16Encoding { field: String, details: String },

    /// Invalid BCD (Binary-Coded Decimal) data
    #[error("Invalid BCD data for {field}: {data:02X?}")]
    InvalidBcdData { field: String, data: Vec<u8> },

    /// Buffer underflow during parsing
    #[error(
        "Buffer underflow at offset {offset}: needed {needed} bytes, only {available} available"
    )]
    BufferUnderflow {
        offset: usize,
        needed: usize,
    ...
    #[error(
        "Command parsing stack overflow at position {position}: maximum depth {max_depth} exceeded"
    )]
    ParseStackOverflow { position: usize, max_depth: usize },
...
impl<T> VgmErrorContext<T> for VgmResult<T> {
        ...
        F: FnOnce() -> String,
    {
        self.map_err(|e| match e {
            VgmError::InconsistentData { context, reason } => VgmError::InconsistentData {
...
mod tests {
    ...
    fn test_error_codes_are_unique() {
        ...
        let errors = vec![
            VgmError::FileNotFound {
                path: "test".to_string(),
                io_kind: None,
            },
            VgmError::FileReadError {
                path: "test".to_string(),
                reason: "test".to_string(),
            },
            VgmError::PermissionDenied {
                path: "test".to_string(),
            },
            VgmError::FileTooSmall {
                path: "test".to_string(),
                size: 0,
            },
            VgmError::InvalidMagicBytes {
                expected: "test".to_string(),
                found: "test".to_string(),
                offset: 0,
            },
            VgmError::CorruptedHeader {
                reason: "test".to_string(),
                offset: 0,
            },
            VgmError::InvalidOffset {
                field: "test".to_string(),
                offset: 0,
                file_size: 0,
            },
            VgmError::TruncatedFile {
                expected: 0,
                actual: 0,
            },
            VgmError::InvalidUtf16Encoding {
            ...
            VgmError::UnknownCommand {
                opcode: 0,
                position: 0,
            },
            VgmError::IncompleteCommand {
                opcode: 0,
                position: 0,
                expected_bytes: 0,
                available_bytes: 0,
            ...
            VgmError::ParseStackOverflow {
                position: 0,
                max_depth: 0,
            },
            VgmError::UnsupportedVgmVersion {
                version: 0,
                supported_range: "test".to_string(),
            },
            VgmError::UnsupportedGd3Version {
                version: 0,
                supported_versions: vec![1],
            },
            VgmError::FeatureNotSupported {
                feature: "test".to_string(),
                version: 0,
                min_version: 1,
            },
            VgmError::MemoryAllocationFailed {
                size: 0,
            ...
            VgmError::ValidationFailed {
                field: "test".to_string(),
                reason: "test".to_string(),
            },
            VgmError::CircularReference {
                structure: "test".to_string(),
                location: "test".to_string(),
            },
            VgmError::InvalidDataBlockType {
                block_type: 0,
                offset: 0,
            },
            VgmError::DataBlockSizeMismatch {
                header_size: 0,
                actual_size: 0,
            },
            VgmError::UnsupportedCompression {
                algorithm: "test".to_string(),
            },
            VgmError::InvalidInputGd3Parser {
                details: "test".to_string(),
            },
            VgmError::FailedParseGd3 {
                reason: "test".to_string(),
            },
        ];
        ...
        assert!(
            codes.len() >= 25,
            "Expected at least 25 unique error types, got {}",
            codes.len()
        );
    }
    ...
    fn test_error_categories() {
        ...
        let file_not_found = VgmError::FileNotFound {
            path: "test".to_string(),
            io_kind: None,
        };
        ...
        let unknown_command = VgmError::UnknownCommand {
            opcode: 0xFF,
            position: 100,
        };
    ...
    fn test_error_recoverability() {
        ...
        let unsupported_gd3 = VgmError::UnsupportedGd3Version {
            version: 999,
    ...
    fn test_error_display() {
        ...
        let command_error = VgmError::UnknownCommand {
            opcode: 0xAB,
            position: 1234,
        };
    ...
    fn test_suggested_actions() {
        ...
        assert!(
            suggestion.to_lowercase().contains("path")
                || suggestion.to_lowercase().contains("exists")
        );
    ...
    fn test_error_category_display() {
        ...
        assert_eq!(
            format!("{}", ErrorCategory::FormatValidation),
            "Format Validation"
        );
        assert_eq!(
            format!("{}", ErrorCategory::CommandParsing),
            "Command Parsing"
        );
        assert_eq!(
            format!("{}", ErrorCategory::VersionCompatibility),
    ...
    fn test_legacy_compatibility() {
        ...
        let gd3_error = VgmError::InvalidInputGd3Parser {
            details: "test".to_string(),
        };
    ...
    fn test_error_debug_formatting() {
        ...
        let error = VgmError::BufferUnderflow {
            offset: 100,
            needed: 10,
            available: 5,
        };
